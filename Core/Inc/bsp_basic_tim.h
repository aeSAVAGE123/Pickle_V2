...
#define GET_BASIC_TIM_PERIOD()      ((__HAL_TIM_GET_AUTORELOAD(&TIM_TimeBaseStructure1)+1)/100.0f)     // 获取定时器的周期，单位ms

#define BASIC_TIM5           		    TIM5
#define BASIC_TIM5_CLK_ENABLE()   	    __HAL_RCC_TIM5_CLK_ENABLE()
...
//				否则TIMxCLK=APB1_CLK * 2
// 设定定时器频率为=TIMxCLK/(TIM_Prescaler+1)=1MHz
#define BASIC_PRESCALER5_COUNT   (72)//1000Khz

extern TIM_HandleTypeDef TIM_TimeBaseStructure1;
extern TIM_HandleTypeDef TIM_TimeBaseStructure5;
