...
int8_t receiving_process(void)
{
    ...
    uint8_t header_CRC_Ready_flat;
    uint64_t tmp_para;            /** 注意，参数的有可能是负数，如果是负数时，传输过来的数据是补码 */
        ...
        else
        {
            ...
            switch (cmd_type)
            {
                case SetPIDValue_CMD:
                {
                    ...
                    /** 提取了命令，解析参数。 */
                    rd_p = (rd_p + 1) % max_length;
                    uint8_t group1Data[4] = {0, data_buff[rd_p + 2], data_buff[rd_p + 1], data_buff[rd_p]};
                    uint32_t group1 =COMPOUND_32BIT(group1Data);


                    /** 更新 rd_p 到下一组数据的起始位置 */
                    rd_p = (rd_p + 3) % max_length;
                    uint8_t group2Data[4] = {0, data_buff[rd_p + 2], data_buff[rd_p + 1], data_buff[rd_p]};
                    uint32_t group2 =COMPOUND_32BIT(group2Data);

                    /** 更新 rd_p 到最后一组数据的起始位置 */
                    rd_p = (rd_p + 3) % max_length;
                    uint16_t group3 = (uint16_t)(data_buff[rd_p + 1] << 8 | data_buff[rd_p]);

                    float p_temp, i_temp, d_temp;

                    p_temp = (float)group1/1000.0f;
